...
import { type CompanyData, getStrapiImageUrl, getCompanyData } from "@/lib/strapi";
import { useStrapiData } from "@/lib/useStrapiData";

export default function CompanyClient({ data: dataProp }: { data: { th: CompanyData | null; en: CompanyData | null; jp: CompanyData | null } | CompanyData | null }) {
  const { t, lang } = useLanguage();
  const fetchCompany = useCallback((locale: string) => getCompanyData(locale), []);
  const data = useStrapiData<CompanyData>(fetchCompany, dataProp);
        ...
        <div className="relative z-10">
          <Container>
            <div className="max-w-2xl">
              <Reveal>
                <h1 className="text-2xl font-bold tracking-tight text-slate-900 sm:text-4xl md:text-5xl drop-shadow-[0_2px_4px_rgba(255,255,255,0.8)]">
                  {heroTitle}
                </h1>
              </Reveal>
              <Reveal delay={0.08}>
                <p className="mt-3 sm:mt-5 text-base sm:text-lg font-normal leading-7 sm:leading-8 text-slate-700 drop-shadow-[0_1px_2px_rgba(255,255,255,0.8)]">
                  {heroSubtitle}
                </p>
              </Reveal>
            </div>
